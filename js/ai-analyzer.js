...
export class AIAnalyzer {
  ...
  // Validate: is this a room photo? Resizes to 512px — only needs low-res to check
  async validatePhoto(file) {
    try {
      const base64 = await resizeMax(file, 512);
      const r = await post(`${this.workerUrl}/validate-photo`, { imageBase64:base64, mimeType:'image/jpeg' });
  ...
  // Full room analysis — resizes photo to max 1200px before sending
  async analyzeRoom(file, roomLabel) {
    try {
      // Resize to max 1200px — Anthropic rejects large images and it costs more tokens
      const base64 = await resizeMax(file, 1200);
  ...
  // AI render — Stability AI SDXL img2img
  async generateRender({ room_type, design_style_id, palette_id, compass_zone, room_sqft, roomPhotoFile }) {
    try {
      ...
      if (roomPhotoFile) {
        // Stability AI SDXL img2img works best with images up to 1024px
        // We preserve the original aspect ratio (unlike SD v1.5 which needed square)
